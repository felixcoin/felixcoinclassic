...
#if defined(NDEBUG)
# error "Felixcoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Felixcoin Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        if (fLimitFree && nModifiedFees < GetFelixcoinMinRelayFee(tx, nSize, !fLimitFree))
        {
...
bool CheckTxInputs(const CChainParams& params, const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight)
{
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if (coins->IsCoinBase()) {
                // Felixcoin: Switch maturity at depth 145,000
...
void ThreadScriptCheck() {
    RenameThread("felixcoin-scriptch");
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // Felixcoin: BIP30 has been active since inception
    ...
    // Felixcoin: BIP16 has been enabled since inception
    ...
    CAmount blockReward = nFees + GetFelixcoinBlockSubsidy(pindex->nHeight, chainparams.GetConsensus(pindex->nHeight), hashPrevBlock);
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // Felixcoin: Disallow AuxPow blocks before it is activated.
    ...
    // Felixcoin: Version 2 enforcement was never used
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindexPrev)
{
    ...
    // Felixcoin: We probably want to disable this
    int nLockTimeFlags = 0;
