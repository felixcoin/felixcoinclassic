...
*Setup instructions for a Gitian build of Felixcoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Felixcoin
...
- [Building Felixcoin Core](#building-felixcoin-core)
...
Clone the git repositories for felixcoin and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/felixcoin/felixcoin.git
git clone https://github.com/felixcoin/gitian.sigs.git
```
...
in the felixcoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Felixcoin Core
----------------

To build Felixcoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the felixcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/felixcoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/felixcoin/felixcoin
...
For example:
```bash
URL=https://github.com/laanwj/felixcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit felixcoin=${COMMIT} --url felixcoin=${URL} ../felixcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit felixcoin=${COMMIT} --url felixcoin=${URL} ../felixcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit felixcoin=${COMMIT} --url felixcoin=${URL} ../felixcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the felixcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../felixcoin/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/felixcoin/felixcoin-detached-sigs.git

BTCPATH=/some/root/path/felixcoin
SIGPATH=/some/root/path/felixcoin-detached-sigs

./bin/gbuild --url felixcoin=${BTCPATH},signature=${SIGPATH} ../felixcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/felixcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/felixcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/felixcoin-osx-build.assert
```
...
[felixcoin/gitian.sigs](https://github.com/felixcoin/gitian.sigs/) repository, or if that's not possible create a pull
