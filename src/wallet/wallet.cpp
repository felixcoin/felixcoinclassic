...
{
    // Felixcoin: Add an increased fee for each dust output
    return std::max(minTxFee.GetFee(nTxBytes) + GetFelixcoinDustFee(tx.vout, minTxFee), ::minRelayTxFee.GetFee(nTxBytes));
}
...
CAmount CWallet::GetMinimumFee(const CMutableTransaction& tx, unsigned int nTxBytes, unsigned int nConfirmTarget, const CTxMemPool& pool, CAmount targetFee)
{
    ...
    // Felixcoin: Drop the smart fee estimate, use GetRequiredFee
